...
const CreateChannelModal = ({ onClose }) => {
    ...
    const [isCreating, setIsCreating] = useState(false); // to prevent multiple submissions
    const [error, setError] = useState(""); // for form validation errors
    const [users, setUsers] = useState([]); // all users that can be added to the channel
    const [selectedMembers, setSelectedMembers] = useState([]);
    const [loadingUsers, setLoadingUsers] = useState(false); // used while fetching users
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            ...
            setLoadingUsers(true); // used for showing loading screen while we fetch users from stream

            try {
                // we want to see everyone except ourselsves
                const response = await client.queryUsers(
                    { id: { $ne: client.user.id } }, // this is the filter for that
                    { name: 1 },
                    { limit: 100 } // custom limit for number of users
                );
    ...
    // custom validation for channel name
